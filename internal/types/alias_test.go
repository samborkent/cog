...
func TestAliasInstantiate(t *testing.T) {
	...
	if alias.TypeArguments[0] != Basics[UTF8] {
...
func TestInstantiatedAliasKindThroughForward(t *testing.T) {
	...
	instantiated := &Alias{
		...
		Derived:       forward,
		Global:        true,
		TypeArguments: []Type{Basics[UTF8]},
	}
...
func TestAliasInstantiateMultipleArgs(t *testing.T) {
	...
	dictDef := &Alias{
		...
		TypeParameters: []*Alias{kparam, vparam},
		Global:         true,
	}
	...
	if len(alias.TypeArguments) != 2 {
		t.Fatalf("TypeArgs len = %d, want 2", len(alias.TypeArguments))
	}
	...
	if alias.TypeArguments[1] != Basics[Int64] {
		t.Errorf("TypeArgs[1] = %v, want int64", alias.TypeArguments[1])
	}
