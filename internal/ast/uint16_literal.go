package ast

import (
	"fmt"
	"strconv"
	"strings"

	"github.com/samborkent/cog/internal/tokens"
	"github.com/samborkent/cog/internal/types"
)

var _ Expr = &Uint16Literal{}

type Uint16Literal struct {
	Token tokens.Token
	Value uint16
}

func (a *AST) NewUint16Literal(t tokens.Token) (ExprIndex, error) {
	value, err := strconv.ParseUint(t.Literal, 10, 16)
	if err != nil {
		return ZeroExprIndex, fmt.Errorf("unable to parse int literal to uint16: %w", err)
	}

	expr := New[Uint16Literal](a)
	expr.Token = t
	expr.Value = uint16(value)

	return a.AddExpr(expr), nil
}

func (l *Uint16Literal) Pos() (uint32, uint16) {
	return l.Token.Ln, l.Token.Col
}

func (l *Uint16Literal) Hash() uint64 {
	return hash(l)
}

func (l *Uint16Literal) StringTo(out *strings.Builder, _ *AST) {
	_ = out.WriteByte('(')
	_, _ = out.WriteString(strconv.FormatUint(uint64(l.Value), 10))
	_, _ = out.WriteString(" : uint16)")
}

func (l *Uint16Literal) String() string {
	var out strings.Builder
	l.StringTo(&out, nil)
	return out.String()
}

func (l *Uint16Literal) Type() types.Type {
	return types.Basics[types.Uint16]
}
